...
class GenerationMixin:
    ...
    def greedy_search(
        self,
        input_ids_unpadded: torch.LongTensor,
        logits_processor: Optional[LogitsProcessorList] = None,
        stopping_criteria: Optional[StoppingCriteriaList] = None,
        ...
        **model_kwargs,
        ...
        if isinstance(eos_token_id, int):
            eos_token_id = [eos_token_id]
        eos_token_id_tensor = torch.tensor(eos_token_id).to(input_ids_unpadded.device) if eos_token_id is not None else None
        output_scores = output_scores if output_scores is not None else self.generation_config.output_scores
        output_attentions = (
            output_attentions if output_attentions is not None else self.generation_config.output_attentions
        )
        ...
        # keep track of which sequences are already finished
        unfinished_sequences = torch.ones(input_ids_unpadded.shape[0], dtype=torch.long, device=input_ids_unpadded.device)
        ...
        for k, t in enumerate(input_ids_unpadded):
            input_ids[k, : len(t)] = torch.tensor(t).long()
        # device = xm.xla_device()
        input_ids = input_ids.to(input_ids_unpadded.device)
        input_text_mask = input_ids != pad_token_id
        model_kwargs["attention_mask"] = input_text_mask #Anisha: overwrite the attention_mask generated by _prepare_attention_mask_for_generation()
        start_pos = min_prompt_size
        self.start_pos = start_pos
        prev_pos = 0        
        self.cur_pos = start_pos #Anisha: store the cur_pos which is the index where the next token will be inserted

        print("Anisha: input_ids.shape={}, input_text_mask.shape={}, self.cur_pos = {}".format(input_ids.shape,input_text_mask.shape,self.cur_pos))
        ...
        while True:
            ...
            # update generated ids, model inputs, and length for next step
            input_ids = torch.cat([input_ids, next_tokens[:, None]], dim=-1)
            #instead of concatenating, we update index
            model_kwargs = self._update_model_kwargs_for_generation(
                outputs, model_kwargs, is_encoder_decoder=self.config.is_encoder_decoder
            )
